...
class vp8 : public ::testing::Test {
 protected:
  virtual void SetUp() {
    ...
    source_file_ = fopen("./test/data/foreman_420_8_176_144_30.yuv", "rb");
    ...
    int stride_uv = 0;
    int stride_y = 0;
    webrtc::Calc16ByteAlignedStride(codec_inst_.width, &stride_y, &stride_uv);
...
TEST_F(vp8, encoder_parameter_test) {
...
#endif
TEST_F(vp8, MAYBE_AlignedStrideEncodeDecode) {
  SetUpEncodeDecode();
...
#endif
TEST_F(vp8, MAYBE_DecodeWithACompleteKeyFrame) {
  SetUpEncodeDecode();
  encoder_->Encode(input_frame_, NULL, NULL);
  EXPECT_GT(WaitForEncodedFrame(), 0u);
