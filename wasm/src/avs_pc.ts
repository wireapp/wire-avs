            ...
            if (track.kind === 'video' && pc.conv_type !== CONV_TYPE_ONEONONE) {
                ...
                for (const trans of transceivers) {
                    if (trans.mid === 'video' && !!trans.sender) {
                        ...
                        const downScaleFactor: DownScaleFactor = determineDownScaleFactor(track, pc.vstate === PC_VIDEO_STATE_SCREENSHARE);
                        const {params, layerFound} = getEncodingParameter(trans.sender, downScaleFactor)
...
type DownScaleFactor = { scaleResolutionDownBy: number, maxBitrate: number };
