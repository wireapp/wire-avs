...
CONTRIB_PARTS_android := \
	BREAKPAD GTEST LIBRE LIBREW OPENSSL
CONTRIB_PARTS_ios := \
	GTEST LIBRE LIBREW OPENSSL
CONTRIB_PARTS_linux := \
	GTEST LIBRE LIBREM LIBREW \
	CONTRIB_OPENSSL
CONTRIB_PARTS_osx := \
	GTEST LIBRE LIBREM LIBREW OPENSSL
...
CONTRIB_GTEST_PATH := $(CONTRIB_BASE)/gtest
CONTRIB_GTEST_BUILD_PATH := $(BUILD_TARGET)/contrib/gtest
...
CONTRIB_OGG_PATH := $(CONTRIB_BASE)/ogg
CONTRIB_OGG_BUILD_PATH := $(BUILD_TARGET)/ogg
CONTRIB_OGG_CONFIG_TARGET := $(CONTRIB_OGG_PATH)/configure
CONTRIB_OGG_TARGET := $(BUILD_TARGET)/lib/libogg.a
CONTRIB_OGG_FILES := $(shell git ls-files $(CONTRIB_OGG_PATH))

CONTRIB_OGG_LIBS := -logg
CONTRIB_OGG_LIB_FILES := $(CONTRIB_OGG_TARGET)

$(CONTRIB_OGG_CONFIG_TARGET):
	cd $(CONTRIB_OGG_PATH) && \
	./autogen.sh

$(CONTRIB_OGG_TARGET): $(TOOLCHAIN_MASTER) $(CONTRIB_OGG_CONFIG_TARGET) $(CONTRIB_OGG_FILES)
	@rm -rf $(CONTRIB_OGG_BUILD_PATH)
	@mkdir -p $(CONTRIB_OGG_BUILD_PATH)
	cd $(CONTRIB_OGG_BUILD_PATH) && \
		CC="$(CC)" \
		CXX="$(CXX)" \
		RANLIB="$(RANLIB)" \
		AR="$(AR)" \
		CFLAGS="$(CPPFLAGS) $(CFLAGS)" \
		CXXFLAGS="$(CPPFLAGS) $(CXXFLAGS)" \
		$(CONTRIB_OGG_PATH)/configure \
			$(CONTRIB_OGG_OPTIONS) \
			--prefix="$(BUILD_TARGET)" \
			$(HOST_OPTIONS)
	$(MAKE) -C $(CONTRIB_OGG_BUILD_PATH) clean
	$(MAKE) $(JOBS) -C $(CONTRIB_OGG_BUILD_PATH)
	$(MAKE) -C $(CONTRIB_OGG_BUILD_PATH) install
...
$(CONTRIB_CRYPTOBOX_TARGET): $(TOOLCHAIN_MASTER) $(CONTRIB_CRYPTOBOX_FILES)
	sed -i.bak s/dylib/staticlib/g $(CONTRIB_CRYPTOBOX_PATH)/Cargo.toml
	...
	cp $(CONTRIB_CRYPTOBOX_PATH)/target/debug/libcryptobox.a \
		$(BUILD_TARGET)/lib/.
	sed -i.bak s/staticlib/dylib/g $(CONTRIB_CRYPTOBOX_PATH)/Cargo.toml
