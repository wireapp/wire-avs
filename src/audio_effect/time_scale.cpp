...
static void time_scale_remove_one(struct time_scale *ts, int best_d, int L)
{
    ...
    int16_t buf1[2*L];
    int16_t buf2[2*L];
    int idx1 = (ts->write_idx - 2*L) & MASK;
    int idx2 = (ts->write_idx - 2*L - best_d) & MASK;
    for(int i = 0; i < 2*L; i++){
        buf1[i] = ts->buf[idx1];
        buf2[i] = ts->buf[idx2];
        idx1 = (idx1 + 1) & MASK;
    ...
    idx1 = (ts->write_idx - 2*L + L1) & MASK;
    idx2 = (ts->write_idx - 2*L + L1 - best_d) & MASK;
...
static void time_scale_insert_one(struct time_scale *ts, int best_d, int L)
{
    ...
    int buf_smpls = (ts->write_idx - ts->read_idx) & MASK;
    if( buf_smpls < L + L1){
        L = buf_smpls - L1;
    }
    int16_t buf1[L+L1];
    int16_t buf2[L+L1];
    int idx1 = (ts->write_idx - L - L1) & MASK;
    int idx2 = (ts->write_idx - L - best_d - L1) & MASK;
    ...
    idx2 = (idx1 - best_d) & MASK;
    for( int i = 0; i < best_d - L1; i++){
        ts->buf[idx1] = ts->buf[idx2];
        idx1 = (idx1 + 1) & MASK;
        idx2 = (idx2 + 1) & MASK;
    }
    for(int i = 0; i < L; i++){
                       ...
                       bool voiced)
{
    ...
    for( int i = 0; i < N; i++){
        ...
        ts->write_idx = (ts->write_idx + 1) & MASK;
    }
}
                        ...
                        int N)
{
    ...
    if(ts->voiced){
        ...
        for(int d = ts->minL; d < (ts->maxL + 1); d++){
            idx1 = (ts->write_idx - 2*L10) & MASK;
            idx2 = (ts->write_idx - 2*L10 - d) & MASK;
            float cc, e1, e2, nc;
            e1 = 0;
            e2 = 0;
            cc = 0;
    ...
    if(buf_smpls > ts->fs_out_khz*45){
        ...
        while(buf_smpls > ts->fs_out_khz*45){
            ...
            buf_smpls = (ts->write_idx - ts->read_idx) & MASK;
        }
    } else {
        /* Buffer is shrinking expand the signal */
        if(buf_smpls < ts->fs_out_khz*35 && best_nc > 0.75f){
            int m = (10*ts->fs_out_khz) -  L10;
            int n = (int)( (1.2f*(float)m) / (float)best_d + 1.0f);
            for(int i = 0; i < n; i++){
    ...
    for( int i = 0; i < N; i++){
        ...
        ts->read_idx = (ts->read_idx + 1) & MASK;
    }
}
