    ...
    for ( auto it = gvoe.nws.begin(); it < gvoe.nws.end(); it++) {
        if ( it->channel_number_ == ch) {
            ...
            for(int i = 0; i < cnt; i++){
                ...
                vst->avg_InVol += it->nw_stats[i].in_vol;
                vst->max_InVol = std::max((uint16_t)it->nw_stats[i].in_vol, vst->max_InVol);
                vst->min_InVol = std::min((uint16_t)it->nw_stats[i].in_vol, vst->min_InVol);
                
                vst->avg_OutVol += it->nw_stats[i].out_vol;
                vst->max_OutVol = std::max((uint16_t)it->nw_stats[i].out_vol, vst->max_OutVol);
                vst->min_OutVol = std::min((uint16_t)it->nw_stats[i].out_vol, vst->min_OutVol);
            ...
            if(vst->num_measurements > 0) {
                ...
                for(int i = 0; i < vst->num_measurements; i++){
                    ...
                    tmp = ((float)vst->avg_InVol - (float)it->nw_stats[i].in_vol);
                    vst->std_InVol += (tmp*tmp);
                    tmp = ((float)vst->avg_OutVol - (float)it->nw_stats[i].out_vol);
